...
namespace System.Numerics
{
    ...
    public readonly struct BigFloat : IComparable, IComparable<BigFloat>, IEquatable<BigFloat>
    {
        ...
        public BigFloat(BigInteger numerator, BigInteger denominator) : this ((numerator, denominator))
        { 
        }
        public BigFloat((BigInteger numerator, BigInteger denominator) rationalNumber)
        {
        ...
        private static (BigInteger, BigInteger) DoubleToRational(double value)
        {
            BigInteger denominator = 1;
            var frac = value % 1;
            while (frac > Double.Epsilon)
            {
                value *= 10;
                denominator *= 10;
                frac = value % 1;
            }
            return (new BigInteger(value), denominator);
        }

        public BigFloat(double value) : this (DoubleToRational(value))
        {
